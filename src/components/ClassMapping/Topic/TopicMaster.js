...
const TopicMaster = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      const apiUrl = `${url}/OfflineTopicMaster`;
      const token = sessionStorage.getItem('token');

      const formDataToSend = new FormData();
      formDataToSend.append('CourseId', formData.class); // Send class as CourseId
      formDataToSend.append('SubjectId', formData.subject);
      formDataToSend.append('LessionId', formData.lesson);
      formDataToSend.append('Topic', formData.topic);
      formDataToSend.append('Content', formData.Content || 0);
      formDataToSend.append('TopicPath', formData.TopicPath || 0);
      formDataToSend.append('VideoLink', formData.VideoLink || 'empty');
  ...
  >
    {topicName.map((item) => (
      <MenuItem key={item.lessionId} value={item.lessionId}> 
        {item.lessionName}
      </MenuItem>
    ))}
  </Select>
